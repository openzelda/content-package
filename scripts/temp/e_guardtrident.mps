...
new WoffsetX[4][4] = { {  1,    1,    1,   2}, {  9,  9,  14,  9}, { 2,  3,  3,  2}, { -17, -17, -22, -17}};
new WoffsetY[4][4] = { {-15,  -14,  -14, -19}, {  8,  8,  6,   8}, { 7,  9,  9, 11}, {  8,  8,   6,  8}};

new Sprites[4][20];	// 4 Strings to hold the sprites for each direction
new Parent[20];		// The guard entitiy who owns this weapon
new adj = 2;
...
main()
{
	if (FirstRun())
	{
		Sprites[north] = "_guardtridentn";
		Sprites[east]  = "_guardtridente";
		Sprites[south] = "_guardtridents";
		Sprites[west]  = "_guardtridentw";
...
public Draw( x, y, depth, direction, frame )
{
	...
	// Draw the weapon
	if ( GetState(Parent) == frozen )  // If the guard is frozen then draw the weapon blue-ish
		PutSprite( Sprites[ direction ], x, y, depth, 0, 100, 100, 255);
	else
		PutSprite( Sprites[ direction ], x, y, depth);
	
	// Set the direction of the weapon to the same as the guard
