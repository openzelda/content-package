...
main() 
{ 
	ConsoleOutput("Map id: '%d' %d", map_id, tstate); 
	//ConsoleOutput("Map id: '%d' - '%d' '%d'", map_id, player_entity, target_entity); 
	if ( tstate ) 
	{ 
		if ( tstate == FADEEND ) 
		{ 
		...
		{ 
			...
			{
				EntityPublicFunction(target_entity, "UpdatePlayer", ''nnn'', _, player_entity,  16.0, 32.0); 
...
{ 
	...
	ConsoleOutput("SetTarget  '%d' ", nmapid); 
